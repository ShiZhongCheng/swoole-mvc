...
class PdoModel
{
    ...
    {
        if (!class_exists("PDO")) {
            throw new Exception("不支持PDO，请先开启", -1);
        }
        $config = $this->getConfig();
        if (isset($config['DB_DEBUG']) && $config['DB_DEBUG'] === true) {
            $this->dbdebug = true;
        }
        // 默认连读库
        $this->setReadConnect();
        // 检测表是否存在
        if (!$this->in_db()) {
            throw new Exception('数据库' . $config['database'] . '中不存在' . $this->table . '表', -1);
        }
    }
    ...
    {
        PdoPool::getInstance()->push($this->link, $this->type);
    }
    ...
    {
        $mysqlConfig = ConfGet::get($this->connection . "." . $this->type);
        $hosts = $mysqlConfig["hostname"];
        $hosts = explode(',', $hosts);
        $count = count($hosts);
        if ($count == 1) {
            $mysqlConfig["hostname"] = $hosts[0];
        } else {
            $mysqlConfig["hostname"] = $hosts[rand(0, $count)];
        }
        return $mysqlConfig;
    }
     ...
     * @return bool
     */
    private function pdoPing(\PDO $link)
    {
        try {
     ...
     * @throws Exception
     */
    public function setReadConnect()
    {
        $this->type = "read";
        $this->getLink();
        return $this;
    ...
    {
        $handle = PdoPool::getInstance()->pop($this->type);
        if (!$handle) {
            connect:
            // 获取配置
            $config = $this->getConfig();
            $config['dsn'] = $config['dbms'] . ':host=' . $config['hostname'] . ';port=' . $config['hostport'] . ';dbname=' . $config['database'];
    ...
    public function where()
    {
        ...
        if (!is_string($where[0]) && !is_array($where[0])) {
            throw new Exception("where子句的参数只支持字符串和数组", -1);
        }
    ...
    public function table($table)
    {
        if (is_string($table)) {
            ...
            if (count($table) == 0) {
                throw new Exception("table子句参数不能传空数组", -1);
            }
            $this->tmp_table = '';
            foreach ($table as $key => $val) {
                if (is_string($key)) {
                    ...
                    if (0 === $match_times) {
                        ...
                        throw new Exception("table子句数组参数的键值非法", -1);
                    }
                } else {
            ...
            throw new Exception("table子句的参数类型错误", -1);
        }
        return $this;
    }
    ...
    public function alias($alias)
    {
        if (is_string($alias) && $alias != '') {
            ...
            throw new Exception("alias子句的参数须是字符串", -1);
        }
        return $this;
    }
    ...
    public function field($field = '', $filter = false)
    {
        ...
        if ($filter === true) {
            if (!is_string($field) && !is_array($field)) {
                throw new Exception("field子句的参数只支持字符串和数组", -1);
            }
            $this->set_columns($this->tmp_table === '' ? $this->table : $this->tmp_table);
            $columns_array = $this->columns;
            unset($columns_array['PRI']);
    ...
    public function order($order)
    {
        if (!is_string($order) && !is_array($order)) {
            throw new Exception("order子句的参数只支持字符串和数组", -1);
        }
        if (is_string($order)) {
            ...
            foreach ($order as $key => $val) {
                if (is_int($key)) {
                    ...
                    if (strtolower($val) != 'desc' && strtolower($val) != 'asc') {
                        $this->orderString = '';
                        throw new Exception("order子句请使用desc或asc关键词指定排序，默认为asc，出现未知字符", -1);
                    }
    ...
    public function limit()
    {
        ...
        if ($param_number == 1) {
            if (!is_int($limit[0]) && !is_string($limit[0])) {
                throw new Exception("limit子句的参数非法", -1);
            }
    ...
    public function page($page_number, $amount)
    {
        if (!is_numeric($page_number) || !is_numeric($amount)) {
            throw new Exception("page方法只支持两个数字参数的写法", -1);
        }
    ...
    {
        if (!is_string($group)) {
            throw new Exception("group子句的参数只支持字符串", -1);
        }
    ...
    {
        if (!is_string($having)) {
            throw new Exception("having子句的参数只支持字符串", -1);
        }
    ...
    public function join($join)
    {
        if (!is_string($join) && !is_array($join)) {
            throw new Exception("join子句的参数只支持字符串和数组", -1);
        }
        if (is_string($join)) {
            $this->joinString .= ' INNER JOIN ' . $join;
        } else {
            if (!is_string($join[0]) || !is_string($join[1])) {
                throw new Exception("join子句中的数组参数的前两个元素必须都是字符串", -1);
            }
    ...
    /**
     * find方法/查询数据(一条)
     * @param string $primary_key_value 用于主键查询
    ...
    {
        // 连主库
        $this->setWriteConnect();
        $field_str = '';
        if ($data != '') {
            if (!is_array($data)) {
                throw new Exception('add方法只支持传入数组', -1);
            }
            $length = count($data);
            if ($length === 0) {
                $placeholder = '';
     ...
     * @return bool|int|null (第一条插入数据的id值)，失败返回false
    ...
    public function addAll($dataList)
    {
        ...
        if (!is_array($dataList)) {
            throw new Exception('addAll方法只支持传入数组', -1);
        }
        $field_str = '';
        $fieldList = array();
    ...
    /**
     * setField方法/更新字段
     * @return int 示例：update users inner join test set user_id='update' where users.id = test.id;
    ...
    public function setField()
    {
        ...
        if ($field === 0) {
            throw new Exception('setField子句须传入参数', -1);
        }
        $this->parseWhere();
        if ($this->whereString == '') {
            $this->set_columns($this->tmp_table === '' ? $this->table : $this->tmp_table);
            if (is_array($field[0]) && isset($this->columns['PRI']) && isset($field[0][$this->columns['PRI']])) {
                if (is_array($field[0][$this->columns['PRI']])) {
                    if (strtoupper($field[0][$this->columns['PRI']][0]) == 'EXP') {
                        ...
                        throw new Exception('setField子句仅支持exp表达式更新', -1);
                    }
                } else {
                ...
                throw new Exception('没有任何更新条件，且指定数据表无主键，不被允许执行更新操作', -1);
            } else {
                throw new Exception('没有任何更新条件，数据对象本身也不包含主键字段，不被允许执行更新操作', -1);
            }
        }
        $setFieldStr = '';
        ...
        if (is_string($field[0])) {
            if ($param_number != 2) {
                throw new Exception('setField子句接收两个参数（属性名，属性值）', -1);
            }
            ...
            if ($param_number != 1) {
                throw new Exception('setField子句只接收一个数组参数', -1);
            }
            ...
            throw new Exception('setField子句传入的参数类型错误', -1);
        }
    ...
    public function save($data)
    {
        ...
        $this->setWriteConnect();
        if (!is_array($data)) {
            throw new Exception('save子句只接收数组参数', -1);
        }
        $this->parseWhere();
        if ($this->whereString == '') {
            $this->set_columns($this->tmp_table === '' ? $this->table : $this->tmp_table);
            if (isset($this->columns['PRI']) && isset($data[$this->columns['PRI']])) {
                if (is_array($data[$this->columns['PRI']])) {
                    if (strtoupper($data[$this->columns['PRI']][0]) == 'EXP') {
                        ...
                        throw new Exception('save子句仅支持exp表达式更新', -1);
                    }
                } else {
                    $this->whereString = ' WHERE `' . $this->columns['PRI'] . '` = ?';
    ...
    /**
     * delete方法/删除数据
     * @param string $table 用于指定主键，删除对应数据
     * @return int
     * @throws Exception
     */
    public function delete($table = '')
    {
        ...
        if ($this->whereString == '') {
            if ($this->joinString == '' || stripos($this->joinString, ' on ') === false) {
                throw new Exception('没有传入任何条件，不被允许执行删除操作', -1);
            }
        }
    ...
    public function execute($execStr, $paramsArray = null)
    {
        if (!is_string($execStr)) {
            throw new Exception('execute查询须传入字符串', -1);
        }
        if ($paramsArray != null) {
            $this->whereValueArray = $paramsArray;
        }
    ...
    public function getLastSql()
    {
        if ($this->dbdebug === false) {
            throw new Exception('请先开启DEBUG模式', -1);
        }
    ...
    public function getLastLog()
    {
        ...
        if (empty($this->config['MySQL_log'])) {
            throw new Exception('尚未指定SQL日志文件的路径', -1);
        }
        ...
        if ($get_file_lastline === false) {
            ...
            if ($is_match != 1) {
                throw new Exception('SQL日志文件最后一行无Query字符串', -1);
            }
            return trim($match[0]);
        }
    }
    ...
    private function parseWhereArrayParam($whereArrayParam)
    {
        ...
        if (isset($whereArrayParam['_logic'])) {
            if (in_array(strtoupper($whereArrayParam['_logic']), $this->SQL_logic)) {
                ...
                throw new Exception('_logic参数指定的逻辑运算符不被支持', -1);
            }
        ...
        if (isset($whereArrayParam['_query'])) {
            ...
            if (isset($explode_array['_logic'])) {
                if (in_array(strtoupper($explode_array['_logic']), $this->SQL_logic)) {
                    ...
                    throw new Exception('_query中的_logic参数指定的逻辑运算符不被支持：', -1);
                }
        ...
        foreach ($whereArrayParam as $key => $val) {
            ...
            if (!is_array($val)) {
                ...
                if ($have_and === false && $have_or === false) {
                    ...
                    //既有&符号，又有|符号
                    throw new Exception('快捷查询方式中“|”和“&”不能同时使用', -1);
                }
            } else {
                $have_and = strpos($key, '&');
                $have_or = strpos($key, '|');
                if ($have_and === false && $have_or === false) {
                    ...
                    if (isset($val['_tosingle']) && isset($val['_tomulti'])) {
                        if ($val['_tosingle'] === true && $val['_tomulti'] === true) {
                            throw new Exception('单条件查询和多条件查询不能同时存在', -1);
                        }
        ...
        switch (strtoupper($array[0])) {
                    ...
                    if (isset($array[2])) {
                        if (in_array(strtoupper($array[2]), $this->SQL_logic)) {
                            ...
                            if (!is_string($array[2])) {
                                throw new Exception('[NOT] LIKE查询中的数组第三个元素必须为字符串，用于指定逻辑运算符', -1);
                            }
                            throw new Exception('[NOT] LIKE查询中的逻辑运算符"', -1);
                        }
                    }
            ...
            case "NOT BETWEEN":
                ...
                if (is_array($array[1])) {
                    ...
                    if (count($explode_array) != 2) {
                        throw new Exception('表达式查询之[NOT]BETWEEN后的参数错误：', -1);
                    }
                    ...
                    if (!isset($array[2]) || !is_numeric($array[2])) {
                        throw new Exception('表达式查询之[NOT]BETWEEN后的参数错误(two number expected)', -1);
                    }
                    $this->whereValueArray[] = $array[1];
            ...
            case "NOT IN":
                ...
                if (is_array($array[1])) {
                    ...
                    if ($length == 0) {
                        throw new Exception('表达式查询之[NOT]IN后的数组参数为空：array()', -1);
                    }
                    $expQueryString .= $column . $string . '(';
                    $expQueryString .= '?';
                    $this->whereValueArray[] = $array[1][0];
                    ...
                    throw new Exception('表达式查询之[NOT]IN后的参数错误：', -1);
                }
                break;
            case "REGEXP":
                $string = ' REGEXP ';
                if (is_array($array[1])) {
                    ...
                    if (isset($array[2])) {
                        if (in_array(strtoupper($array[2]), $this->SQL_logic)) {
                            ...
                            if (!is_string($array[2])) {
                                throw new Exception('[NOT] REGEXP查询中的数组第三个元素必须为字符串，用于指定逻辑运算符', -1);
                            }
                            throw new Exception('[NOT] REGEXP查询中的逻辑运算符"', -1);
                        }
                    }
                    foreach ($array[1] as $val) {
                        $expQueryString .= $logic . $column . $string . ' ?';
            ...
            case "EXP":
                if (is_string($array[1])) {
                    ...
                    throw new Exception('表达式查询之exp后的参数错误：', -1);
                }
                break;
            default:
                throw new Exception('表达式查询之表达式错误："', -1);
        }
    ...
    private function replaceSpecialChar($pattern, $replacement, $subject)
    {
        if (is_array($replacement)) {
            ...
            throw new Exception('replaceSpecialChar函数的第二个参数类型错误', -1);
        }
        return $subject;
    }
    ...
    public function showError()
    {
        if ($this->SQLerror == null) {
            throw new Exception('没有开启DEBUG模式无法看到详细错误信息，或者最近一次SQL操作并没有发生错误', -1);
        } else {
            throw new Exception('Error Code: ' . $this->SQLerror['errno'] . '<br/>SQLSTATE: ' . $this->SQLerror['sqlstate'] . ' <br/>Error Message: <div>' . $this->SQLerror['msg'] . '</div><br/>Error SQL: <div>' . $this->SQLerror['sql'] . '</div>', -1);
        }
    }
